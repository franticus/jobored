...
export const Filters = () => {
  ...
  const spheresUrl = 'https://startup-summer-2023-proxy.onrender.com/2.0/catalogues';

  useEffect(() => {
    axios.get(spheresUrl, {
    ...
    <div className={s.filters}>
      ...
      <div className={s.spheres}>
        ...
        <select className={s.select}>
          ...
          {
            data.map((sphere, i) => (
              <option value="sphere.title" key={i}>{sphere.title}</option>
            ))
          }
        </select>
      </div>
        ...
        <select className={s.select}>
          <option value="" disabled selected>От</option>
          {
            salaryFrom.map((salary, i) => (
              <option value={salary} key={i}>{salary}</option>
            ))
          }
        </select>
        <select className={s.select}>
      ...
      <Button color="primary" text="Применить" size='lg'/>
    </div>
  )
}
